...
/learn                                        ← Learning hub landing
  /learn/challenges                           ← Technical challenges (all domains)
    /learn/challenges/:domain                 ← Domain page (categories list)
      /learn/challenges/:domain/:category     ← Category challenges list
        /learn/challenges/:domain/:cat/:slug  ← Single challenge detail
  /learn/ai                                   ← AI learning curriculum
    /learn/ai/:topicSlug                      ← Single topic module
  /learn/interview-prep                       ← Level-based interview prep
    /learn/interview-prep/:level              ← Level detail (topics list)
      /learn/interview-prep/:level/:topic     ← Topic questions
...
### 3.2 Technical Challenges Module

This is a generic, **admin-extensible** practice hub. It is NOT limited to DSA. Admins can create any domain, any category within it, and any challenge within that category — through the DB. The initial seed covers the following domains, but the system must support arbitrary expansion without code changes:

**Initial domains (seeded by intern team):**
- DSA & Algorithms — practice coding problems
- Design Patterns — GoF and modern patterns
- Low Level Design — OOP and class design problems
- Database Design — schema and query design
- SOLID Principles — clean code and architecture
- Behavioral Interview — STAR method structured practice
- AI & AI Agents — LLMs, RAG, agents, tool use

> **Architecture principle**: Domains, categories, and challenges all live in the DB. Adding a new domain or challenge requires only a DB insert — zero code changes. The frontend and backend are completely content-agnostic.

#### 3.2.1 Challenges Landing (`/learn/challenges`)

- Page title and subtitle (intern team writes the copy)
- Search bar — searches across domain names and descriptions
- **Domain Cards Grid** (3 per row on desktop, 2 tablet, 1 mobile):
  - Domain icon (from `icon` field — Lucide icon name)
  - Domain name
  - Short description of what this domain covers
  - Total challenge count (sum across all categories in this domain)
  - "Explore" button → `/learn/challenges/:domainSlug`

#### 3.2.2 Domain Page (`/learn/challenges/:domain`)

URL pattern: `/learn/challenges/:domainSlug`

- Breadcrumb: `Challenges > [Domain Name]`
- Domain title + description
- Back button
- **Category Cards** (grid or list — intern team decides layout):
  - Category name
  - Challenge count badge
  - Short description
  - "View challenges" button → `/learn/challenges/:domain/:category`

#### 3.2.3 Category Challenges List (`/learn/challenges/:domain/:category`)

URL pattern: `/learn/challenges/:domainSlug/:categorySlug`

- Breadcrumb: `Challenges > [Domain] > [Category]`
- Category title
- Back button to domain page
- List of challenge cards (vertical list), each showing:
  - Difficulty badge (Easy=green, Medium=yellow, Hard=red)
  - Challenge title
  - One-line description
  - "View challenge" button → `/learn/challenges/:domain/:category/:slug`

#### 3.2.4 Single Challenge (`/learn/challenges/:domain/:category/:slug`)

**Breadcrumb**: `Challenges > [Domain] > [Category] > [Challenge Title]`

The challenge detail page layout adapts to the domain type. A DSA problem shows code solutions; a Behavioral question shows a STAR framework; a Design Pattern shows a UML-style breakdown. The schema supports all of these via flexible content fields.

**Page sections (vertical scroll):**

1. **Challenge Header**
   - Title, Difficulty badge, Domain badge, Category badge

2. **Problem / Prompt**
   - Full challenge statement (the question or scenario)

3. **Examples / Scenarios** (where applicable)
   - Input / Output / Explanation for code challenges
   - Scenario / Context / What to address for design or behavioral challenges

4. **Approach / Framework** (the main teaching section)
   - Overview paragraph — how to think about this type of challenge
   - Step-by-step breakdown:
     - Each step: title + explanation + code snippet (language tabs) or diagram description

5. **Complexity / Trade-offs** (where applicable)
   - For code: Time and Space complexity with explanation
   - For design: Trade-offs of the approach

6. **Key Insights**
   - Bullet list of things worth remembering, common mistakes, applicability

7. **Solution / Reference Answer** (where applicable)
   - Language tabs for code challenges (JavaScript / Python / Java)
   - Structured written answer for design/behavioral

8. **External Resources**
   - Links to relevant practice platforms or reading material

9. **Related Challenges**
   - Links to other challenges in the same category
...
### 4.6 Challenge Domain Schema

Top-level grouping (DSA, Design Patterns, LLD, etc.). Admins create these in the DB — no code changes needed to add a new domain.

**File**: `challengeDomainSchema.js`

```javascript
const challengeDomainSchema = new mongoose.Schema({
  name: { type: String, required: true, trim: true },
  // e.g., "DSA & Algorithms", "Low Level Design", "Behavioral Interview"
  slug: {
    type: String,
    required: true,
    unique: true,
    lowercase: true,
    trim: true
    // e.g., "dsa", "low-level-design", "behavioral"
  },
  description: { type: String, trim: true },
  // Short description shown on the challenges landing
  icon: { type: String, trim: true },
  // Lucide icon name, e.g., "Code2", "Layers", "Users"
  order: { type: Number, default: 0 },
  // Controls display order on landing page
  isActive: { type: Boolean, default: true },
  // Admin can hide a domain without deleting it
  createdAt: { type: Date, default: Date.now },
  updatedAt: { type: Date, default: Date.now }
});
...
### 4.7 Challenge Category Schema

Second level — sub-groupings within a domain. e.g., within DSA: Arrays, Trees, Graphs. Within Design Patterns: Creational, Structural, Behavioral.

**File**: `challengeCategorySchema.js`
...
const challengeCategorySchema = new mongoose.Schema({
  ...
  // Denormalized count — update when challenges are added/removed
  isActive: { type: Boolean, default: true },
  createdAt: { type: Date, default: Date.now },
  updatedAt: { type: Date, default: Date.now }
});
...
### 4.8 Challenge Schema

Individual challenges. The content fields are flexible enough to support code problems (DSA, LLD), design questions, and behavioral questions without separate schemas.

**File**: `challengeSchema.js`
...
const challengeSchema = new mongoose.Schema({
  ...
  // Examples/scenarios — for code: input/output; for design/behavioral: scenario/context
  examples: [{
    label: { type: String, trim: true },
    // e.g., "Example 1", "Scenario", "STAR Example"
    input: { type: String, trim: true },
    output: { type: String, trim: true },
    explanation: { type: String, trim: true }
  }],

  // How to approach this challenge (not the answer, the thinking framework)
  approach: {
    overview: { type: String, trim: true },
    steps: [{
  ...
  // Trade-offs / complexity — used for code and design challenges
  tradeoffs: {
    timeComplexity: { type: String, trim: true },
    // e.g., "O(n)" — leave empty for non-code challenges
    ...
    // Bullet points for design/architecture challenges
  },

  keyInsights: [{ type: String, trim: true }],

  // Full reference answer — code or written
  referenceAnswer: [{
    language: {
      type: String,
      enum: ['javascript', 'python', 'java', 'cpp', 'sql', 'text'],
    ...
    tabLabel: { type: String, trim: true },
    content: { type: String }
    // For code domains: actual code. For others: structured written answer.
  }],
...
### 4.10 Interview Prep Topic Schema
...
### 5.2 Challenges Routes (`/api/challenges`)

| Method | Endpoint | Auth | Description |
|---|---|---|---|
| GET | `/api/challenges/domains` | Public | List all active domains |
| GET | `/api/challenges/domains/:domainSlug` | Public | Get domain + its categories |
| GET | `/api/challenges/domains/:domainSlug/:categorySlug` | Public | Get category + paginated challenges |
| GET | `/api/challenges/:domainSlug/:categorySlug/:challengeSlug` | Public | Get single challenge (full detail) |
| POST | `/api/challenges/domains` | Admin | Create a new domain |
| POST | `/api/challenges/categories` | Admin | Create a category within a domain |
| POST | `/api/challenges` | Admin | Create a challenge |
| PATCH | `/api/challenges/domains/:slug` | Admin | Update domain |
| PATCH | `/api/challenges/:slug` | Admin | Update challenge |

**Query params for category challenges list:**
```
?difficulty=Easy&page=1&limit=10
```

> The route design is flat — the full domain/category/challenge context is always available in the slug params without needing nested sub-routes.
...
│   └── LearnPages/
│       ├── LearnHub.jsx                  ← /learn (landing, optional)
│       ├── ChallengesLanding.jsx         ← /learn/challenges
│       ├── ChallengeDomainPage.jsx       ← /learn/challenges/:domain
│       ├── ChallengeCategoryPage.jsx     ← /learn/challenges/:domain/:category
│       ├── ChallengeDetailPage.jsx       ← /learn/challenges/:domain/:category/:slug
...
│   └── LearnComponents/
│       ├── ChallengeDomainCard.jsx       ← Domain card on landing
│       ├── ChallengeCategoryCard.jsx     ← Category card on domain page
│       ├── ChallengeCard.jsx             ← Individual challenge in list
│       ├── ChallengeCodeBlock.jsx        ← Code viewer with language tabs
│       ├── AiTopicCard.jsx
│       ├── TierSection.jsx
│       ├── InteractiveStepper.jsx
│       └── LanguageTabsCode.jsx          ← Reusable across challenges + AI topics
├── services/
│   ├── companyApi.js                     ← API functions + React Query hooks
│   ├── challengesApi.js                  ← Covers domains, categories, challenges
│   └── aiTopicsApi.js
└── store/
    └── slices/
        ├── companyFilterSlice.js         ← Filters for company listing
        └── challengeFilterSlice.js       ← Difficulty filter + pagination for challenges
```
...
// Challenges Hub (generic — covers DSA, Design Patterns, LLD, etc.)
...
<Route key="challenges" path="/learn/challenges" element={<ChallengesLanding />} />
<Route key="challenge-domain" path="/learn/challenges/:domain" element={<ChallengeDomainPage />} />
<Route key="challenge-category" path="/learn/challenges/:domain/:category" element={<ChallengeCategoryPage />} />
<Route key="challenge-detail" path="/learn/challenges/:domain/:category/:slug" element={<ChallengeDetailPage />} />
...
□ Copy model files to /backend/models/
  ...
  - challengeDomainSchema.js
  - challengeCategorySchema.js
  - challengeSchema.js
  - aiTopicSchema.js
